...
def is_past_time(time_str, end_date):
    return datetime.datetime.strptime(time_str.to_list()[0], '%Y-%m-%d').date() <= end_date
...
if __name__ == '__main__':
    ...
    for stock_code in failed_stock_df['stock_code'].values:
        try:
            ...
            daily_final_df = pd.read_csv(save_file_name) if os.path.exists(save_file_name) else None

            start_date = datetime.datetime.strptime(kline_daily_failed_date[0], '%Y-%m-%d').date()
            res, data = data_extractor.get_kline_data(stock_code, start_date, today, 'daily')
            data = data.reset_index().rename(columns={'index':'date'}).astype({'date' : str})
            if res == True:
                status_df['kline_daily'] = status_df[['trading_date']].apply(lambda x: True if is_past_time(x, today) else False, axis=1)
