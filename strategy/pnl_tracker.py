class PnlTracker():
    def __init__(self, start_money):
        ...
        self.total_profit = 0
        self.total_profit_money = 0
        self.max_loss = 0
        self.max_profit = 0
        self.avg_cost = 0

    def buy(self, price, volume=0):
    ...
    def sell(self, price, volume=0):
        ...
        if self.prev_assets > self.current_assets:
            ...
            self.max_loss = self.max_loss if self.max_loss > (self.prev_assets - self.current_assets) else (self.prev_assets - self.current_assets)
        else:
            self.total_profit += 1
            self.total_profit_money += (self.current_assets - self.prev_assets)
            self.max_profit = self.max_profit if self.max_profit > (self.current_assets - self.prev_assets) else (self.current_assets - self.prev_assets)
        return 0
